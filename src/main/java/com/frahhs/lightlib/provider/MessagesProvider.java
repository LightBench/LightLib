...
public class MessagesProvider {
    ...
    public MessagesProvider(JavaPlugin plugin) {
        this.plugin = plugin;
        this.lang = LightPlugin.getInstance().getConfigProvider().getString("language");
        this.prefix = LightPlugin.getInstance().getConfigProvider().getString("prefix");
        this.languageConfigs = new HashMap<>();

        loadLanguageFiles();
        if (!languageConfigs.containsKey(lang)) {
            LightPlugin.getLightLogger().warning("Language \"%s\" not found! English automatically selected.", lang);
            lang = "en";
        }
    }
    ...
    public String getMessage(String key, boolean usePrefix) {
        ...
        if (config == null) {
            LightPlugin.getLightLogger().warning("Language files '%s' not found.", lang);
            return null;
        }
        ...
        if(!config.contains(key)) {
            ...
            if(!configEng.contains(key)) {
                LightPlugin.getLightLogger().error("The message path '%s' was not found, empty value used. Try to regen the lang folder.", key);
            } else {
                LightPlugin.getLightLogger().warning("The lang path '%s' does not exist for the language '%s', english used.", key, lang);
    ...
    private void loadLanguageFiles() {
        ...
        for (String curYML : languagesYMLFileName) {
            File curYMLFile = new File(LightPlugin.getInstance().getDataFolder(), "lang/" + curYML);
            if (!curYMLFile.exists())
                LightPlugin.getInstance().saveResource("lang/" + curYML, false);
        }
    ...
    private List<String> getResources(String path) {
        ...
        try {
            ...
            LightPlugin.getInstance().getServer().getConsoleSender().sendMessage("ERROR - getResources() - couldn't decode the Jar file to index resources.");
        } catch (IOException ex) {
            LightPlugin.getInstance().getServer().getConsoleSender().sendMessage("ERROR - getResources() - couldn't perform IO operations on jar file");
        }
    ...
    private static class YamlUpdater {
        ...
        public static void update() {
            try {
                ...
                LightPlugin.getLightLogger().warning("Error while trying to update language files, be sure to have the latest version of LightPlugin installed.");
            }
        }
